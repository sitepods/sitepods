...
export default function CityHero({ cityName, state, headline, subtext, areas }: Props) {
  return (
    <section className="relative overflow-hidden hero-bg">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="pt-28 pb-16 lg:pt-36 lg:pb-20 max-w-3xl">
          ...
          >
            ...
            <span className="text-xs font-semibold text-indigo-700 dark:text-indigo-300">
              {cityName}, {state} — Website Design Service
            </span>
          </motion.div>
          ...
          <motion.h1
            ...
            className="text-[2rem] sm:text-[2.4rem] lg:text-[2.8rem] font-extrabold text-gray-900 dark:text-white leading-[1.2] tracking-tight mb-4"
          >
            {headline.split(cityName).map((part, i, arr) =>
              i < arr.length - 1 ? (
                <span key={i}>
                  {part}
          ...
          <motion.p
            ...
            className="text-[0.95rem] text-gray-500 dark:text-gray-400 leading-relaxed mb-6 max-w-[560px]"
          >
            {subtext}
          </motion.p>
          ...
          >
            {areas.map((area) => (
              <span
                key={area}
                className="text-[11px] font-medium px-2.5 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800"
              >
                {area}
              </span>
            ))}
          ...
          >
            ...
            >
              {cityName} mein Website Chahiye <ArrowRight className="w-4 h-4" />
            </Link>
            <Link
              href="/pricing"
